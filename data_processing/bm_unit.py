class BMUnit:
    ...
    def get_remaining_volume_by_pair(self):
        ...
        max_energy_export = self.physical_volumes[ct.ElexonDatasetNames.MELS.value]
        max_energy_import = self.physical_volumes[ct.ElexonDatasetNames.MILS.value]
        scheduled_energy_delivery = self.physical_volumes[ct.ElexonDatasetNames.PN.value]
        for pair_index in range(len(positive_pair_ids)):
            energy_from_previous_offers = sum([self.bid_offer_pairs_submitted[pair_id][ct.ColumnHeaders.LEVEL_FROM.value] for pair_id in positive_pair_ids[:pair_index]])/2
            available_energy_this_offer = self.bid_offer_pairs_submitted[positive_pair_ids[pair_index]][ct.ColumnHeaders.LEVEL_FROM.value]/2
        ...
        for pair_index in range(len(negative_pair_ids)):
            energy_from_previous_bids = sum([self.bid_offer_pairs_submitted[pair_id][ct.ColumnHeaders.LEVEL_FROM.value] for pair_id in negative_pair_ids[:pair_index]])/2
            available_energy_this_bid = self.bid_offer_pairs_submitted[negative_pair_ids[pair_index]][ct.ColumnHeaders.LEVEL_FROM.value]/2
