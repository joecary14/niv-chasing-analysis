...
async def get_ancillary_price_data_for_sp_calculation(
    ...
    mid_data = await get_full_midp_data(api_client, settlement_dates_with_periods_per_day.keys())
    missing_mid_data = check_missing_data(mid_data, settlement_dates_with_periods_per_day)
    missing_data.update(missing_mid_data)
    
    price_adjustment_data = await get_price_adjustment_data(['settlement_date', 'settlement_period', 'buy_price_price_adjustment', 'sell_price_price_adjustment'], settlement_dates_with_periods_per_day.keys(), api_client)
