...
async def get_market_index_price_data(
    ...
    apx_grouped_data = apx_data.copy()
    apx_grouped_data['apx_weighted_price'] = apx_grouped_data['price'] * apx_grouped_data['volume']
    apx_grouped_data = apx_grouped_data.groupby(['settlement_date', 'settlement_period']).agg(
        apx_total_volume=pd.NamedAgg(column='volume', aggfunc='sum'),
        apx_total_weighted_price=pd.NamedAgg(column='apx_weighted_price', aggfunc='sum')
    )
    apx_grouped_data['apx_midp'] = apx_grouped_data['apx_total_weighted_price'] / apx_grouped_data['apx_total_volume']
    apx_result = apx_grouped_data.reset_index()[['settlement_date', 'settlement_period', 'apx_midp']]
